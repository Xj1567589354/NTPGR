...
具体来说是，HRNet+多级上采样机制+各分支浅深层尺度cat拼接+Non-Local
"""
...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(GLBottleBlock, 64, 4)
        ...
        final_inp_channels = sum([352, 192, 256, 256])
        ...
        # # Non-Local attention
        # self.non_local_atte0 = NONLocalBlock2D(in_channels=352)
        # self.non_local_atte1 = NONLocalBlock2D(in_channels=192)
        # self.non_local_atte2 = NONLocalBlock2D(in_channels=256)
        # self.non_local_atte3 = NONLocalBlock2D(in_channels=256)
                ...
                for j in range(i+1-num_branches_pre):
                    ...
                    conv3x3s.append(
                        nn.Sequential(
                            ...
                            nn.ReLU(inplace=True),
                            # # EPSA注意力模块
                            # PSAModule(inplans=outchannels, planes=outchannels)
                        )
                    )
    ...
    def forward(self, x):
        ...
        y_list[0] = torch.cat((x, y_list2[0], y_list3[0], y_list[0]), dim=1)
        y_list[1] = torch.cat((y_list2[1], y_list3[1], y_list[1]), dim=1)
        y_list[2] = torch.cat((y_list3[2], y_list[2]), dim=1)
...
def get_pose_net_morecat(cfg, is_train, **kwargs):
